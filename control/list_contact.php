...
{
  if($_POST['full_name']!="") $search_condition.=" AND contacts.name LIKE '%$_POST[full_name]%'";
  if($_POST['contact_no']!="") $search_condition.=" AND contacts.mobile_no_with_country_code LIKE '%$_POST[contact_no]%'";
}

$sql_info="SELECT contacts.* FROM contact_list contacts WHERE 1 $search_condition ORDER BY contacts.id DESC";
$msg="";
$head="All Contacts";
...
<div class="wapper">
  ...
  <div class="contentarea">
    ...
    <div>
      <form id="frm_send_sms" name="frm_send_sms" action="" method="post">
        <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td width="19%"> 'Name:</td>
            <td width="31%"><input type="text" name="full_name" id="full_name" /></td>
            <td width="23%">Phone No:</td>
            <td width="27%"><input type="text" name="contact_no" id="contact_no" /></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
    ...
    <div id="show_dealer_listing">
      <!--//**** Top Pagination *******-->
      <h3><?php echo $total_rows;?> Contacts found</h3>
      ...
      <table width="30%" border="0" cellpadding="0" cellspacing="0">
        <thead>
          <tr>
            <td width="20%" align="center">S/N</td>
            <td width="30%">Contact No</td>
			<td width="20%">Status</td>
            <td width="30%">Manage</td>
          </tr>
        </thead>
        ...
        <tr id="arrayorder_<?php echo $row['id'];?>">
          ...
          <td><?php echo $row['mobile_no_with_country_code'];?></td>
		  <td><?php if($row['is_stop']=="0") echo "UNSUBSCRIBED"; else echo "SUBSCRIBED";?></td>
          <td><a href="delete_contact_by_id.php?id=<?php echo $row['id']?>" class="sbtn_red">Delete</a> <a href="new_contact.php?eid=<?php echo $row['id'];?>" class="sbtn_green">Edit</a></td>
        </tr>
        <?php } ?>
        <?php }else{ ?>
        <tr>
          <td colspan="6" align="center">Sorry. No Contact found.</td>
        </tr>
        <?php } ?>
      </table>
    </div>
