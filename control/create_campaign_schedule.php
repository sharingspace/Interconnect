...
$sql_all_contact="SELECT * FROM contact_list WHERE is_stop='1'";
$q_all_contact=mysql_query($sql_all_contact);
$total_contact=mysql_num_rows($q_all_contact);
...
if(isset($_POST['btnSend']))
{
  ...
  elseif(trim($_POST['sms_text'])!="")
  {
	  //Read csv file
	  $msg="";
	  $successfully_scheduled=0;
	  $successfully_sms_scheduled=0;
	  while ($row=mysql_fetch_array($q_all_contact))
	  if($row['mobile_no_with_country_code']!="")
	  {
		  $phone_no=trim(str_replace("-","",$row['mobile_no_with_country_code']));
...
<div class="wapper">
  ...
  <div class="contentarea">
    ...
    <form name="frm_admin" id="frm_admin" method="post" action="<?php echo $base_admin.$current_page_name;?>" enctype="multipart/form-data" class="fields">
      <p> You have total <?php echo $total_contact;?> contacts.<br />
      </p>
      <br />
      <p>
      ...
      <p>
        ...
        <br />
        <br />
        <span class="infobar">[Once you press <?php echo $lbl;?> button system will automatically create a schedule to send this question to all <a href="list_contact.php" target="_blank">contacts</a> from database. Total contact <?php echo $total_contact;?>. ]</span> <br /><br />
